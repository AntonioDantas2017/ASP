...
<div align="center"> 
  ...
  <table width="700" border="0" cellspacing="0" cellpadding="0" class=linka>
    <tr> 
      <td height="13" bgcolor="#CCCCCC"><div align="center"> <strong>CADASTRO DE COZINHA
      </strong></div></td>
    </tr>
  </table>
  <table width="700" border="0" cellspacing="0" cellpadding="0">
...
call abre_conexao01
	set objRs01 = objConn01.execute("select max(codcoz) as maxcodcoz from cozinha")
	if IsNull(objRs01("maxcodcoz")) then
	   codcoz = 100000
	else
	   codcoz = objRs01("maxcodcoz") + 1
	end if
	
	sql = "INSERT INTO cozinha (codcoz,valor,data,obs) values ('"&codcoz&"','"&valor&"',#"&data&"#,'"&obs&"') "
	'response.write sql	
...
%>
                ...
                <br>
                O cadastro do registro foi realizado com sucesso!<br>
                </strong>
				<br>			
                    <br>
                    <table width="50%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td><div align="center"><a href="cozinha.asp?listar=<%=listar%>&ordem=<%=ordem%>&pesquisa=<%=pesquisa%>&tipo=<%=tipo%>&filtro=<%=filtro%>&pesquisa2=<%=pesquisa2%>&datainicio=<%=datainicio%>&datafim=<%=datafim%>&filtro2=<%=filtro2%>"><img src="IMAGEM/botoes_principal.gif" alt="volta para a p&aacute;gina principal" width="75" height="19" border="0"></a></div></td>
                      </tr>
                    </table>
                    <br>
                    <br>
